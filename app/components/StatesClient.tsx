...
export default function StatesClient({ statesData }: StatesClientProps) {
  ...
  // Define Mock regions for grouping states
    ...
    >
      ...
      <div className="page-header text-center mb-12">
        ...
        <p className="page-subtitle text-lg text-gray-500 max-w-2xl mx-auto">
          Select a state or union territory to discover its distinct capital, languages, cultural landmarks, and districts.
        </p>
      </div>
          ...
          {filteredStates.map((state: StateData, index: number) => (
            <Link href={`/states/${state.id}`} key={state.id}>
              <motion.div
                ...
                className="card glass-panel h-full flex flex-col hover:-translate-y-2 transition-all duration-300 overflow-hidden bg-white"
              >
                {/* State Banner Image */}
                <div className="h-44 overflow-hidden relative">
                  <RotatingImage 
                    searchTerm={`${state.name}, India`}
                    defaultImages={state.images || [state.image]} 
                    alt={state.name} 
                  />
                ...
                <div className="p-5 flex flex-col flex-grow">
                  <h2 className="text-xl font-extrabold mb-1 text-gray-800 flex items-center gap-1.5 hover:text-accent-primary transition-colors">
                    {state.name}
                  </h2>
                  
                  {/* Capital / Language quick badges */}
                  <div className="flex gap-2 mb-3 text-[11px] text-gray-500 font-semibold">
                    <span className="flex items-center gap-0.5 bg-gray-100 px-2 py-0.5 rounded">
                      Cap: {getCapitalName(state.name)}
                    </span>
                  </div>

                  <p className="text-gray-500 text-xs mb-4 leading-normal flex-grow line-clamp-3">
                    {state.description}
                  </p>
                  ...
                  <div className="mt-auto pt-3 border-t border-gray-100 flex items-center justify-between">
                    ...
                    <span className="text-xs font-bold text-accent-primary flex items-center gap-0.5 group-hover:translate-x-1 transition-transform">
                      Explore Hub →
                    </span>
                  </div>
                </div>
